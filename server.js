...
setInterval(() => {
    http.get(`http://himadabah7788.glitch.me/`); /// حط اسم المشروع تبعك name تعديل مهم بدل
}, 280000);
...
client.on("message", async message => {
    ...
    if (message.content.startsWith(prefix + "bc")) {
        ...
        let args = message.content
            ...
            .slice(1)
            .join(" ");
        if (!message.author.id === "735316948036092171") return; ///تعديل مهم حط الايدي تبعك
...
client.on("ready", () => {
    client.user.setActivity(`Barlin Server q{prefix}bc`, { ///تعديل مهم حالة البوت
        type: "WATCHING"
    }); ///تعديل حالة البوت
});
